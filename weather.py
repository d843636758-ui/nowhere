...
_QWEATHER_HOST: str = os.environ.get("NOWHERE_QWEATHER_HOST", "")
...
async def _try_qweather(lat: float, lon: float) -> dict[str, Any] | None:
    ...
    QWeather began phasing out the legacy shared API hosts in 2026, so
    NOWHERE_QWEATHER_HOST should be set to the account-specific API Host
    shown in the QWeather developer console, for example:
    ...
    # Accept either a bare hostname or a pasted https:// URL.
    host = host.removeprefix("https://").removeprefix("http://").rstrip("/")

    # QWeather documents coordinate precision up to two decimal places.
    ...
    try:
        ...
        # QWeather windSpeed is km/h; Nowhere's field is m/s.
