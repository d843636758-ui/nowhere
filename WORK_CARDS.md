...
# 内容引擎组(2026-08-20,旋复:"优化了行走又如何,没东西玩"——动词只有6个,互相不咬合)

## 卡41:卡里的人(455 地卡的角色变成立体的人)— 最大的空缺

只许改:nowhere/server.py、nowhere/encounters.py、新建 nowhere/data/people_seed.json、nowhere/tests/test_people.py(新建)

原则:真实旅行=人;卡里已经有人("老茶客""萨米人""摊主"),让布景活过来。**不新写 NPC 库,从已有卡里挖。**

1. **种子抽取**(一次性脚本活,进 people_seed.json):扫 localcolor/humanities 全部卡,抽"卡里提到的人"——(地,角色,卡的原文上下文)。每地挑 1-2 个最立体的。首版 60 人起步(喀什茶馆的老茶客/拉普兰的驯鹿牧人/威尼斯贡多拉船夫/廷巴克图的盐商后人……),照 WRITING_PROMPT 声口,每人:
   - 名字(当地感的,不真名)
   - 在场理由(和原卡同一处)
   - 三句话:一句应景的 / 一句只有本地人才知道的(可以从 zim 实据里提炼) / 一句私人的(有缺口的话,不煽情)
   - 一个"只有他知道的事"(附近有什么/节日什么时候/路怎么走——**接进差事和问路系统**)
2. **遇见**:walk 落到该地该卡附近(5km)→ 40% 出遇见段(人+动作,不自我介绍:"茶馆门口坐着个老人,茶碗在他手里转。")。
3. **搭话**:新工具 `talk(person: str | None = None)`——不指名就搭最近遇见过的人;返回他的一句(三句池轮换,同人不重复句,见过 3 句后他记得你:"又是你。"变体)。**人不是信息亭**:他说的指向世界(方向/传闻/节日),不总结剧情。
4. 离开该地 30km,人留下;重游时痕迹链(卡10)推进后,人的第三句可换("茶馆翻新完那年他就不来了"——人也会离开,变体)。
5. test_people.py:喀什落地走到茶馆附近能遇见;talk 三句不重复且第四句是"又是你"类;人的话指向世界不指向玩家情绪。

验收:新测试绿;3 个地的遇见+搭话全流程贴报告。

---

## 卡42:差事发生器(动词咬合:事本身就是报酬,无奖励系统)

只许改:nowhere/server.py、nowhere/state.py、nowhere/journeys.py、新建 nowhere/errands.py、nowhere/tests/test_errands.py(新建)

三种差事,全是已有动词的组合,不新造机制:

1. **邮差**:postcard 扩展——寄信时 10% 概率收到"带信"请求(信封=一张卡:收信地是特征描述"有灯塔的地方"/"产葡萄酒的镇",不是地名);旅者查图/问人(卡41 的"只有他知道的事")→ walk_to → 到了 `deliver()`:交给该地遇见的人(卡41)或埋下。信送掉,回执进 journal(卡8)。**没奖励,送达即结束。**
2. **寻宝链**:卡13 的 bury 支持留"指向下一站的谜语"(不给坐标,给特征:"三江并流的地方,石头比水硬");发现铁盒的人可以接棒。链长 2-3 站,终点埋一张白卡(留给人写)。同链全局唯一。
3. **节日追逐**:卡11 落地时若 800km 内有 5 天内开幕的节日,电台/问人处可得风声(walk/listen 10% 触发一次,"电台里在说 X 天后南边的{place}有个节");去不去旅者自己定,赶上了就是卡11 的节日落地,赶不上痕迹链记一笔。
4. state 加 `errand`(当前差事,序列化;一旅程同时最多一个,拒绝不惩罚——"你把信收进包里。哪天顺路再说。"变体)。
5. test_errands.py:带信请求→deliver 闭环;谜语链两站接力;节日风声在窗口内才出现。

验收:新测试绿;邮差全流程贴报告(带信→查→走→送)。

---

## 卡43:旅行手账(自然志,外部记忆,不是成就)

只许改:nowhere/placememory.py、nowhere/server.py、新建 nowhere/tests/test_notebook.py(新建)

1. placememory 层(全局跨旅程)加 notebook 结构,分册自动记:**植物册**(baked flora 的学名+俗名,初见时地)/**动物册**(encounters life)/**电台册**(台名+_genre+在哪听的)/**水域册**(water_features 分段名)/**人册**(卡41 遇见搭过话的)。每条 {名, 俗名, 首见地, 首见时(模拟时刻)}。
2. 新工具 `notebook()`——按册点名,自然志声口变体:"植物册记了 43 笔。第一笔是云莓,在拉普兰,那时是夏天。"/稀罕条目单列:"只有一次的:极光下的电台。"(出现次数=1 的条目归'只有一次的'节)。
3. 记录时机:各渲染路径出卡时顺笔(不改卡逻辑,埋点收名)。**不弹提示、不加分数、无解锁音效——手账安静地长。**
4. test_notebook.py:抽卡后 notebook 有笔;同种只记首见;跨旅程不清零;模拟时刻正确。

验收:新测试绿;一段含植物+电台+遇见的旅程后,notebook() 输出贴报告。

---

## 卡44:同游者(共享世界:人和人的交叉是唯一的内容乘法器)

只许改:nowhere/placememory.py、nowhere/server.py、nowhere/web.py、新建 nowhere/tests/test_cotravelers.py(新建)

背景:placememory/明信片/埋藏已是全局共享——这游戏天然是异步多人。把它点亮。

1. **旅者注册表**(placememory 层):travelers.json——{名字, 当前地/坐标, 上次活跃}。open_door 时自动报到(NOWHERE_TRAVELER_NAME 环境变量,缺省"无名旅者";各 AI 用自己的名字:有一/九九/花园的机号)。
2. **脚印可见**:_record_footprint 已在记——walk 时 3km 内有**别人** 24h 内的脚印 → 15% 一句("沙面上有一串脚印,不是你的。方向朝北。"),data 带谁的(不点名:"有人比你先到"变体池;重游交叉多了才点名"这脚印你认得,是{名}的")。
3. **相遇**:两人 3km 内且都在活跃 → walk 返回里各给一句相遇(不强制互动,各写各的:"河边坐着另一个旅者,你们点了下头。"——谁也不打扰谁,Camino 式)。
4. **留话给人**:post_message(网页,人写的)本就进 messages;扩一层:埋藏物/明信片的 note 里 @名字,被点名旅者下次 open_door 收到一句转达("有人给你留了句话,在{place}的土里。")。
5. test_cotravelers.py:两个 traveler 名交替 open_door/walk,脚印句触发;@留言送达;注册表更新。
