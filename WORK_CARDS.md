...
## 卡40:诚实的边界(软边界三件套,替代"回原地"哲学)— 立刻可发

只许改:nowhere/walk.py、nowhere/server.py、nowhere/state.py、nowhere/tests/test_boundary.py(新建)

背景:旋复:"不可能无限探索,所以只好写让他回原地,但不好。"业界三条路:地理边界(岛的形状)/软边界(让玩家自己回头)/静态骨干+程序血肉(Caves of Qud:历史村庄走到哪生成到哪)。乌有乡零件全有,只缺接线。**原则:不许再用假动作("往回走,回到原地")糊弄任何探索。**

1. **内容密度衰减**(软边界核心):
   - 落点周边 30km 内有 explorable/hydro 分段/localcolor 键 → 密度正常;
   - 30-100km → 遭遇概率×0.5,叙事变疏(变体池:"好久没见着人迹了。");
   - >100km 荒深 → 遭遇×0.2,进入"荒深档"渲染:天、地、身体三件为主,世界安静但不空洞(禁"什么都没有"——用天光/风/地面质感撑,变体池 8 句照声口);
   - walk_to 输出预计到达信息时,若目的地在密度稀区,如实报"那边是荒地,要走 80 公里"——**探索预算交还旅者**,由他决定回头还是深走。
2. **球面 wrap**:walk 经度过 ±180 自动 wrap(现在会怎样先探一下,炸/漂移都修);纬度过 ±85 极地 → "再往前没有北了"式收束句(变体 3 条)+允许掉头。**地球没有边,只有绕回来。**
3. **悬崖/水域阻挡如实化**:cliff blocked 已有;walk 朝开阔水面 ≥5km → "前面是水面,过不去"(如实),游泳模式仅当旅者明确要(walk 继续顶着走第二次才切 water mode 并给涉水文案)。
4. **深荒回报**(Qud 哲学:走到哪生成到哪):荒深档连走 10 步以上,低概率(5%)出"程序血肉"事件——按当前坐标从 zim/geocode 就近拉一个真实小地名/小特征,渲染成发现("地图上这里有个叫X的小地方,你走到时只剩三户人家"式,数据没有就生成本地特征:一棵孤树/一段旧路基/一处泉,变体池 12 句)。**生成的发现引用真实数据优先,编造兜底也要是"世界的"不是"系统的"。**
5. state 加 `wilderness_depth_km`(随 walk 更新,序列化),look/where_am_i 在荒深档如实报"你在离任何名字都远的地方"。
6. test_boundary.py:密度三档遭遇概率单调递减(mock rng 统计);经度 wrap 后坐标合法;±85 收束句触发;深荒 10 步后 5% 事件可 mock 命中且引用真实地名;输出永不包含"回到原来的地方"。

验收:新测试绿;从任意落点朝一个方向连走 15 步贴报告——密度衰减可感、无假动作、荒深档不空洞。

---

# 内容引擎组(2026-08-20 v2 定稿,旋复:"内容要着墨很详细很好,不能模模糊糊")

数据底盘(脑已勘明,executor 不必重查):localcolor.json 153 地的卡里含活人(巴格达铜匠/格拉纳达吉普赛舞者/瓦尔帕莱索壁画艺人);humanities.json 409 地每地已有 `人物` 卡(真实历史人物带声口,如喀什的阿曼尼莎汗);encounters.txt 按洲分池。声口一律照 docs/WRITING_PROMPT.md(卡31)。

## 卡41:卡里的人(两层人:卡中人与故人)

只许改:nowhere/server.py、新建 nowhere/data/people_seed.json、新建 nowhere/people.py、nowhere/tests/test_people.py

**人的两层定义(不许混):**
- **卡中人(虚构,活跃的)**:从 localcolor 卡里挖出的无名者——铜匠、老茶客、赶驯鹿的。他们活在"现在",能遇见能搭话。**不取真名**,名字是当地感的称呼(卡孜姆大爷/老于尔根/西格娜阿姨),免得真人背台词。
- **故人(真实,历史的)**:humanities `人物` 卡里的人(阿曼尼莎汗/班超)。**不可搭话**——他们不在了。遇见方式是"痕迹式":走到相关地,遇一段他们的在场复现(卡文本已有,直接用),可 `ask(name)` 深挖(zim,已有链路)。**这一层零新数据。**

**people_seed.json 格式(卡中人,首版 60 人,挑 60 个 localcolor 有人味的地)**:
```json
{"喀什": {"person": "卡孜姆", "where": "老茶馆门口",
  "sight": "茶馆门口坐着个老人,茶碗在他手里转,茶要凉了他也不喝。",
  "lines": [
    "茶馆的墙又刷了一遍。刷完还是老样子,热。",
    "巴扎的杏干要买周三的,周三的新。别问为什么,就是周三。",
    "我儿子在乌鲁木齐。这条巷子他小时候天天跑。"
  ],
  "knows": {"type": "direction", "text": "上山的小路在清真寺后面,车进不去。"},
  "months": [3,4,5,6,7,8,9,10]}}
```
字段硬规矩:
- `sight`:一句**在动作中**的他(拿/转/擦/看),禁站桩介绍,禁外貌清单。
- `lines[3]`:一句应景的(和当天 env 呼应可留钩)/一句本地知识(具体到可执行:周几、哪条路、哪个时辰,从 zim 实据提炼,不许编)/一句私人的(一个缺口,不解释,不煽情)。
- `knows`:direction(路)/festival(节日风声,接卡42)/rumor(附近传闻,指向差事)三型,是问路的答案源。
- `months`:他不在的月份不遇见(卡孜姆冬天去乌鲁木齐带孙子——人就该有来去)。

**遇见规则**:walk 落在该地 5km 内 → 40% 出 `sight`(同一次旅程只出一次,再次路过他可能不在:20%)。
**talk(person=None)**:不指名=最近遇见的。返回 `lines` 轮换(同人不重复,顺序 1→2→3);三句后第四句是记得你(变体池 4:"又是你。坐。"——仍然指向动作,不抒情);`talk("路怎么走")` 类关键词 → 答 `knows`。**人不是信息亭**:每句 ≤40 字,说的指向世界,不评论旅者。
**故人路径**:humanities `人物` 卡在遇见时照常出;出卡后返回文本尾带一句引导("这名字你记下了。ask 能问出更多。")——只在人物卡后加,事件/作品卡不加。

**test_people.py**:喀什 5km 内 40% 遇见可 mock;三句轮换不重复+第四句记得;months 外不遇见;knows 三型各有路由;故人卡尾引导只在人物类。

验收:新测试绿;报告贴 3 地全流程(遇见→三句→第四句)+1 个故人卡。

---

## 卡42:差事发生器(三种差事,完整规格)

只许改:nowhere/server.py、nowhere/state.py、新建 nowhere/errands.py、nowhere/data/errands_letters.json、nowhere/tests/test_errands.py

**设计公理:事本身就是报酬。无奖励、无完成音效、无进度条。拒绝不惩罚,搁置永远可以。**

**1. 邮差(主力差事,触发+流程全规格)**:
- **触发**:寄出 postcard 后 10%(一旅程最多接一封在身);或问卡中人 `knows.rumor`("北边有人托我带过一封信,一直没顺路")→ talk 追问接信。
- **信的内容(errands_letters.json 首版 20 封)**:不是任务书,是**一封真的短信**——寄信人一句+托带一句,声口照 WRITING_PROMPT。样例(写进卡的范例,照此水平写 20 封):
  - "给我妹妹。她在海边守灯塔那镇上。就说,妈的病好多了。——于尔根,克里斯蒂安桑"
  - "带给酿酒的那家人。今年收成好,欠的桶钱开春还。——一个不想留名的人"
  - "交给温泉边开旅馆的。这封信迟到三年了,不怪我。——邮政所最后的员工"
  收信地一律**特征不地名**:守灯塔的镇/产葡萄酒的谷/温泉边/有渡口的江边——候选从 explorable_index + water_features_offline(卡37 接线后可用)反查真实匹配地,谜面写时就锚定唯一真地(灯塔→至少一个真实灯塔地存在)。
- **寻找**:旅者问卡中人(`knows.direction`/`festival` 型回答往候选地方向指)或 `ask(特征)`(zim 链路已有)。**系统不验证找得对不对**——走错了信还在包里。
- **送达 `deliver()`**:落在候选地 5km 内 → 成(交给该地卡中人:出一段交接两行,信人各一句,不煽情;该地无人则埋下);回执一行进 journal(卡8):"X月X日,于尔根的信送到了{地}。晚了 11 天。"(晚的天数=模拟时间差,如实)。
- **在身感受**:信在包里时,wait 长夜文案 10% 混入一句信的重量("包里的信纸摩擦出一点声音。")(变体 4)。

**2. 寻宝链(bury 的接力扩展)**:
- `bury(note="…")` 的 note 若含"下一个"(谜语式特征指向另一地,如"去三江并流的地方,石头比水硬"),埋下时生成链 ID,全局 buries 记链。
- 发现者 `dig()`(新,对应卡13 的发现动作)读 note → 可 `carry()` 接棒(当前 souvenir 换成"一个铁盒,里面有张字条")→ 到谜底地埋下 → 链推进。链长上限 3 站,第 3 站 bury 的 note 留空盒白卡("盒子空着,留给下一个写字的人")。
- 谜面校验: bury 时 note 若像谜语(无地名直给),系统反查是否存在至少一个匹配地,没有则提示"这谜太死了,没人解得开"(不阻断,只提示)。

**3. 节日追逐(风声,不是任务)**:
- 落地时 800km 内有 ≤5 天(模拟时间)开幕的节日(卡11 数据)→ walk/listen 各 10%(每旅程一次)出风声:"电台里在说,四天后南边的{place}有{节日}。"
- 无接取/无追踪。赶上了=卡11 节日落地自然发生;没赶上=痕迹链(卡10)记一笔("你到的时候,地上还有彩纸")。
- where_am_i 带 errand 在身时附一行:"你带着一封信,去{方向}。"(只报方向,不报进度)

**state/序列化**:`state.errand = {"kind": "letter", "from": "...", "text": "...", "hint": "...", "taken_at": 模拟时刻}` | {"kind": "chain", "id": ..., "leg": 2} | None。一旅程最多一个;搁置(`errand` 留着)/转交(埋下信,变 chain 第一站)都合法。

**test_errands.py**:接信→问人指路→走到→deliver 回执进 journal 全链(mock 坐标);谜语链两站接力+第 3 站空盒;风声只在窗口内;信在身 wait 混入;搁置/转交合法。

验收:新测试绿;报告贴一封信的完整一生(接→问→走→交→回执)。
...
**notebook(册名可选)输出声口(变体池,自然志不是报表)**:
- 开册:"植物册,43 笔。第一笔是云莓,拉普兰,那时是夏天。最近一笔是昨天的。"(有首有近,中间不点名)
- 指定册:全列,每条一行"云莓——拉普兰,夏天"(季节从 at 反推,南半球翻转已有先例可抄)。
...
## 卡44:同游者(异步多人,完整规格)— ★默认关,opt-in

**总开关(旋复拍板 2026-08-20:"不要默认开,有人可能更想自己玩")**:
- 一切同游功能(注册/脚印/相遇/@转达)由 `NOWHERE_COTRAVEL=1` 环境变量开启,**默认关闭**——关着时单人世界,不留任何"别人存在过"的痕迹文案, travelers.json 不写不读,零开销。
- 开关粒度:主开关之外可配 `NOWHERE_COTRAVEL=q uiet`(只脚印不见面/不点名)。单人玩家的世界必须和今天完全一样。
- 开着也尊重退出:`walk_alone()` 工具——本次旅程屏蔽同游文案(注册表保留,标记"独行"),之后 `open_door` 恢复。想彻底退:环境变量去掉重启。
- 文档:README 或 docs/TASTE_LOOP.md 旁加一节"同游怎么开、开了会发生什么、怎么关"——让选择是知情的。

只许改:nowhere/placememory.py、nowhere/server.py、nowhere/web.py、新建 nowhere/travelers.py、nowhere/tests/test_cotravelers.py

**注册表 travelers.json(placememory 层,全局)**:
```json
{"又又": {"place": "长江·三峡段", "pos": [30.8, 111.0], "last_seen": "...", "door_count": 12},
 "有一": {"place": "喀什", "pos": [39.5, 76.0], "last_seen": "...", "door_count": 3}}
```
- 报到:open_door 成功落地时写(名字取 NOWHERE_TRAVELER_NAME 环境变量;web.py 网页端落地也写,用 session 给的或"网线那头的人")。walk 每 5 步刷新 pos。7 天不活跃移出主表(进归档,不算消失——归档旅者的脚印句换过去式:"这脚印旧了,雨都下过一场了")。
- **名字可见性**:同游者的存在感按距离递进——5km 内才点名,5-30km 只说方向,30km 外沉默。**世界不播报谁上线了。**

**脚印(最重要,旋复的 _record_footprint 已经在记)**:
- walk 时 3km 内有**别人** 24h(模拟时间)内的脚印 → 15% 一句。方向照脚印真实走向。
- 递进:首次交叉(和这人第一次踩见)→ 不点名("沙面上有一串脚印,不是你的。朝北去了。");同一人第 3+ 次交叉 → 点名("这脚印你认得了——是有一的,他又在往山里走。")。
- 变体池 6 句起步,按 surface 变(沙地/雪地/湿地/城内各至少 1)。

**相遇(同步时刻,克制)**:
- 条件:两人 24h 内都活跃且当前相距 <3km → 各自 walk 返回里给一句,各写各的视角,互不引用对方的文本(谁也不知道对方怎么写自己——**这是刻意的,留白**)。
- 样例(写进卡):"河边坐着另一个旅者,你们点了下头。各自看各自的水。"
- 频率:同对旅者 7 天(模拟)最多相遇一次。不触发对话——想说话走 @留言。

**@留言(异步传话)**:
- 格式:bury 的 note / postcard 正文里 `@名字` → 被点名者下次 open_door 落地后首条 walk 返回附一句转达:"土里有人给你留了话,在{place}。是谁,得自己去看。"(变体 3;不代读内容——话要自己挖,埋在哪句里已说了)
- @不存在的旅者:静默不报错(写错名字的信寄丢,正常)。

**test_cotravelers.py**:双旅者(monkeypatch 环境变量)交替 open_door/walk——注册表各自更新;3km 脚印句+方向正确;第 3 次交叉点名;@留言跨旅者送达;7 天活跃窗口过期;网页端落地也注册。

验收:新测试绿;报告贴双旅者交叉全剧本(甲乙各 5 步的返回原文)。

---

## 卡45:内容加厚(2600 张,按普查靶子分三批)— 依赖卡34 普查报告

只许新建:docs/WRITING_PROMPT_v2.md、docs/THICKEN_PLAN.md;不改代码

**为什么这张卡主要是写 prompt(旋复:"prompt 要好好写,非常影响质量")**:455 地工程的最大教训——同一个模型,prompt 平了热干面就是"碱水面煮熟淋上芝麻酱",prompt 立了就是"芝麻酱裹在面上,你拌三下才匀,嚼起来嘎吱响"。prompt 差一个字,两千张卡差一个档次。

**WRITING_PROMPT_v2.md 的规格(在 78f67bd 版 STYLE_GUIDE 基础上升级,原文所有铁律保留)**:

1. **质量地板原文照搬**(廷巴克图/阿瑜陀耶两段+六条示范讲解)——这是旋复定过的地板,一个字不许动。
2. **新增:加厚专用条款**(v1 是"重写一个地方",v2 是"往一个已经不错的地方**添卡**",难度更高——添的卡不许和已有卡撞角):
   - **开工前先读该地全部已有卡**(数据在 localcolor.json),列一张"已写过的角度"清单——写新卡前逐条避开。已写过"清晨的巴扎",新卡就不许再从清晨的巴扎进。
   - **新卡的切入角清单**(每地至少用到 4 种不同角):一个时刻(不是清晨黄昏这两个俗角,写"下午四点下学时"/"打烊前半小时")/一种天气(雨里的/起风时的)/一类人(摆摊的/赶路的/守夜的)/一件小事(称重的动作/找零的手势)/一样声音(一种鸟/一种机器)/身体的一个部位(脚底/后颈/指腹)。
   - **同地异卡的最大敌人是"总结式写法"**——一旦开始总结("喀什是丝绸之路的重镇"),就写不出第二张不同的卡了。每张卡必须钉死在一个具体瞬间。
3. **新增:节律卡专节**(这次的主攻维度,13 地 → 80 地):
   - 节律卡是"时间限定卡",格式 {"hours":[起,止), "text":"…", "months":[…](可选)};写法不是"晚上的城市很热闹",是"二十二点的巴扎,收摊的人把毯子卷起来,卷到最后一张,扬出一片灰,灯一盏一盏灭"。
   - **months 季节限定是硬货**:极光 9-3 月、白夜 6-7 月、雨季/旱季、渔汛、花期、雪季、节庆前夜——每个有季节现象的地至少 1 条 months 卡。写前查该地真实气候(zim 实据),不许凭感觉。
   - 南半球翻转提醒写进 prompt(悉尼 12 月是夏天)。
4. **新增:防"模板回潮"条款**(量产到第 8 批时最容易犯):
   - "空气厚得像布"这类地板里的好比喻,**只许在示范里出现,不许被抄进新卡**(比喻要为当下的卡现造)。
   - 每写 25 张,回头读自己第 1 批的 5 张——句式结构撞了就停,换角。
   - 禁词表加"一次/一阵/一片"滥用监控(连续 5 张都用"一片"开头就该警觉)。
5. **自查清单升级**(写完每地必跑):禁词/空泛词扫描、R1-R6≥4、hours 整数、months 该带没带漏、**新卡与已有卡角度不撞**(人工过一遍)、四维度看得见。

**THICKEN_PLAN.md 的规格(生产计划,等卡34 报告填数)**:
- 批次 1(最差 40 地):重写+各 +10 张,每地一名"主笔"(一个 subagent 从头到尾),不许两地以上流水线轮写——连续性是声口的命。
- 批次 2(名城 10 地:北京/巴黎/伦敦/罗马/上海+普查点名的):各 +10 张,重点补节律 months 和"深夜档"(现在全球深夜卡最薄)。
- 批次 3(节律专项):40 个有季节现象的地各 +2-3 条 months 卡(极北/赤道/季风区/地中海优先)。
- 每批的验收:抽 10% 过 LLM 裁判(卡24 golden),平均分 <3.5 的批打回;**主笔制下同地卡片风格一致性人审**(同一支笔写出的一个地方才像一个地方)。
- 节奏:每批 20 地一停,旋复读 3 张(盲评,卡36),她说继续再继续——**品味回路嵌进量产,不是量产完再补救**。

验收:两份文档成文;WRITING_PROMPT_v2 含地板原文+v2 四个新增章节+自查清单;THICKEN_PLAN 三批的靶子/主笔制/验收线/暂停点写清。产出仅文档,不动数据。

---

## 卡46:六根时间轴(农历/天象/物候/星期/生物钟/纪念日)— 脑自发,旋复"你自己发散"

总纲:**内容不够,先让时间变厚——同一个地 365 天没有两天相同,同样的卡数翻出几倍的"不同"。**落地时 state.now() 进各轴判定,命中出卡;六轴独立,可叠加(周日+英仙座极大夜+农历七月半)。全部只动渲染判定层,不新造系统。

**按性价比分六子卡,可分开发**:

**46a 周律(最便宜,先做)**:weekday 进 env 候选——
- 周五晚 18-24:城变松(变体池 5,世界通用);"周末前夜"在写字楼区/大学城的差异(靠 biome)。
- 周日早 6-12:教堂钟声(欧美城)/公园晨练(东亚城)/空街(商业区)。
- **赶集日**(中国):农历一四七/二五八/三六九赶集——集日表按地区常见惯例粗定(乡镇 biome + 华北/西南),集日 8-14 时出集市卡(变体池 6,竹筐/活禽/称重的手)。周一:博物馆图书馆"闭馆"卡(biome=city 且有 art_by_city 条目的地,低调一句)。
- 卡文案 30 张起步,只作 open_door 签名行的时段词变化 + walk 低频插入,不许每步刷存在感。

**46b 农历时Clock(一根轴带三件事)**:接 lunardate 或 zhdate 库(pip,离线,纯推算)——
- 农历日期进 env;节日判定(卡11 中国系)改挂农历轴(春节=正月初一,弃公历表)。
- **潮汐联动**:农历初一/十五 ±1 天 = 大潮。海边(距海<10km)walk/listen 的海洋卡换大潮变体(变体池 4:"潮比平时高半米,礁石都吞了");初八/廿三小潮反之。
- **月相**已经有 sky.py 数据,农历轴让文案能说"十五的月亮"而不是裸数字。
- 中元(七月半)/中秋等夜卡在对应农历日强化权重。

**46c 天象历(一张表)**:meteor_showers.json——英仙(8/12±3)、双子(12/13±3)、象限仪(1/3±3)、猎户(10/21±3)、宝瓶η(5/5±3),各带:极大夜日期/辐射点星座/北半球可见度分级/每小时天顶流量(ZHR 档:大/中/小)。判定:夜 + 日期窗 + 天气非阴雨 → walk/look 抬头 25% 出流星卡(档位分文案,变体池 8:"一颗,划过去就没了。你还没来得及许愿,它已经不在了。");极大夜落地加一张(ZHR 大才配)。日食月食:2026-2035 可见表(罕见,优先级低,先留位)。

**46d 生物钟历(months 维度扩展)**:encounters 侧加"时令动物"——候鸟(鄱阳湖白鹤 11-3/崇明东滩鸻鹬 4-5·9-10)、萤火虫(6-7 月 20-22 时,森林/湿地,一份"萤火虫时刻"卡值得专门写)、三文鱼洄游(阿拉斯加 9-10)、海龟产卵(加勒平/东南亚海滩 6-9 夜)、帝王蝶(墨 11-3,已在卡11)。格式:在相应数据源加 months 字段(和节律卡同构),渲染时过滤。**这是"不是节日的'现在正好'"——撞上纯属幸运,幸运就是记忆。**

**46e 物候历(粗表,不精算)**:phenology.json——按 月+纬度带(寒温/暖温/亚热/热)× 半球 的矩阵,每格 1-2 个物候现象(华北 4 月杨絮/江南 6 月梅雨/热带 4-5 月芒果花/南半球 9-10 蓝花楹)。命中 → walk 低频插一句(变体按现象写,首批 30 现象)。**目的:随机落点也有季节感,落在 5 月华北就是杨絮季,不靠节日也成立。**

**46f 纪念日层(地方的记忆)**:humanities 事件卡已有 year,补 month/day(可空的精度档:年/年月/年月日);落地日期 = 当地事件周年 ±0(精确档)→ 低概率(15%)出一张:"今天,1945 年的这一天,战争在这结束。街上的人不怎么提,但花圈在。"(按事件类型分声口:战争/灾难=克制不消费,文化事件=可温暖)。**权重必须低**——纪念日不喧宾夺主,撞上算地方的私语。数据补全走 add_place 流水线(卡30)顺笔。

**通用规矩**:六轴全部进 where_am_i 的 data(不进 text——时间是给人感受的,不是报时);各轴卡文案照 WRITING_PROMPT_v2;互斥/叠加的优先级:节日(卡11) > 纪念日 > 天象 > 物候/生物钟 > 周律 > 默认——同一步最多叠 2 层,第 3 层让位(信息过载 = 死)。
测试:各子卡独立 test;农历推算对 3 个已知日(2026 中秋=农历八月十五=公历 9/25 验证);潮汐/流星/萤火虫的月份窗判定。
...
1. festivals.json,**首版 86 个节日原型 × 每个 2-3 张卡 ≈ 200 张**(旋复点名中国太少,重盘后的谱系):
   - **中国系 46 个**(占比过半,对得起本土玩家):
     - 全国大节 12:春节/元宵/清明/端午/七夕/中元/中秋/重阳/冬至/腊八/小年/除夕
     - **节气卡 8**(中国独有的时间感,别家没有):立春咬春/谷雨喝茶/立夏称人/夏至面/立秋贴秋膘/霜降柿子/小雪腌菜/冬至数九——节气是"全国准节日",country=CN 全境生效
     - **地域奇节 20**:哈尔滨采冰/自贡灯会/潮汕营老爷/佛山行通济/西双版纳泼水/凉山火把/苗姊妹节/那达慕/新疆诺鲁孜/泉州蟳埔簪花围/潍坊风筝/平遥社火/大理三月街/拉萨雪顿/西江苗年/婺源晒秋/乌镇香市/溱潼会船/佛山秋色/侗族大歌节
     - 汉文化圈 6:香港太平清醮/台湾盐水蜂炮/平溪天灯/冲绳大拔河/首尔燃灯/河内Tết
   - **世界系 40 个**:南亚东南亚 7(洒红/排灯/大壶/卫塞/水灯天灯/巴厘静居日/帕坦因陀罗节)、中东非洲 8(诺鲁孜/开斋/宰牲/斋月夜生活/阿舒拉/阿布辛贝太阳节/埃塞Timkat/马里沙漠音乐节/摩洛哥Gnawa)、欧洲 10(威尼斯/科隆/巴塞尔狂欢、法雅/圣周/奔牛/番茄、仲夏、啤酒节、Hogmanay、希腊火箭教堂复活夜、诺丁山)、美洲 8(亡灵/拉迪什/帝王蝶/里约/萨尔瓦多/Inti Raymi/Oruro/乌拉圭Candombe)、东亚非汉 3、大洋洲极地 4(毛利Matariki/极夜/午夜太阳/极光季)
   - 每个节日先查实据(zim/维基)再写——尤其地域奇节的**具体仪式动作**(营老爷怎么营/蜂炮怎么炸/采冰怎么采),细节错一个,当地人都看得出来。

   格式:
```json
[{"name": "泼水节", "place": "清迈", "window": {"start": [4, 13], "end": [4, 15]},
  "cards": ["满城都是水。……", "……"],
  "eve_cards": ["节日前夜,城里已经在……"]},
 {"name": "开斋节", "country": "伊斯兰历换算", "window": {"years": {"2026": [3, 20], "2027": [3, 9]}},
  "cards": [...]},
 {"name": "樱花", "place": "日本", "lat_rule": {"base_lat": 31.0, "base_date": [3, 24], "days_per_deg": 2.6, "span_days": 10}, "cards": [...]}]
```
三种窗口:(a) 固定日期+place;(b) **伊斯兰历/农历类:逐年换算表**(春节/开斋节/宰牲节/复活节全属此类,查 2026-2035 真实日期填表,严禁写死公历);(c) 纬度推移(樱花/红叶前线,南半球红叶 -days_per_deg)。每种至少 3 实例。

**节日卡的写法规范(这是本卡的灵魂,照此水平写)**:
- **身体先到,名字后到**:第一句必须是被节日击中的身体("一瓢水从后脖颈灌进来,你整个人缩了一下——满街都在笑,没有一个人躲。"→ 这才是泼水节的开法;"水从头顶浇下来"都嫌温)。禁"XX节到了/这里是XX节的现场"式报幕。
- **每张卡是节日的一个瞬间,不是节日的介绍**:泼水节三张=巷战那一刻/躲进屋檐看别人挨浇/傍晚湿着衣服吃糯米;亡灵节三张=万寿菊铺到墓地的路/深夜墓地里 Families 守着蜡烛/清晨扫完的糖骷髅。**一个节日至少三面,不许写成一种气氛。**
- **人要在过节,不是在演给你看**:本地人各自的动作(小孩的水枪/老太往你手里塞花/搬梯子挂灯的人),禁"人们载歌载舞"式群演腔。
- **eve_cards(节日前夜)**:每个节日 1 张——前夜的城是节日最美的一刻(灯笼刚挂/锅已经开了/花还堆在板车上),给"赶上前夕"的旅者。
- **禁词/禁百科照旧**:"泼水节是傣族新年"枪毙;实据查 zim/facts(节日真实细节:亡灵节的三层祭坛/排灯节的排数),编造的具体细节一律不写。
- **文化敬畏线**:宗教节日(开斋/宰牲/复活/卫塞)写**旁观的 respectful**(寺外的鞋阵/排队等开斋的人的脸/钟声),不写嬉闹,不进神圣区域,不消费他人信仰;亡灵节写"人家在祭祖,你在看"的位置感。
