...
# ── Migration (one-time, in-place) ──────────────────────────────────


def migrate_localcolor(data_dir: pathlib.Path | None = None) -> int:
    """Migrate localcolor JSON files to Card-array format.

    Writes back to the same files.  Returns total card count.
    """
    d = data_dir or _DATA_DIR
    cards = load_localcolor(d)
    # Group by source file
    main_places: set[str] = set()
    main_file = d / "localcolor.json"
    if main_file.exists():
        main_places = set(json.loads(main_file.read_text("utf-8")).keys())

    regional_map: dict[str, set[str]] = {}
    regional_files = {
        "localcolor_china.json": set(),
        "localcolor_japan_korea_sea.json": set(),
        "localcolor_americas_africa_oceania.json": set(),
        "localcolor_natural.json": set(),
        "localcolor_europe_middleeast.json": set(),
    }
    for fname in regional_files:
        p = d / fname
        if p.exists():
            regional_files[fname] = set(json.loads(p.read_text("utf-8")).keys())

    # Group cards by source file
    main_cards: list[dict] = []
    regional_cards: dict[str, list[dict]] = {f: [] for f in regional_files}

    for card in cards:
        place = card.conditions.get("place", "")
        card_dict = _card_to_dict(card)
        if place in main_places:
            main_cards.append(card_dict)
        else:
            for fname, places in regional_files.items():
                if place in places:
                    regional_cards[fname].append(card_dict)
                    break

    # Write back
    _write_cards_json(main_file, main_cards)
    for fname, rcards in regional_cards.items():
        p = d / fname
        if p.exists() and rcards:
            _write_cards_json(p, rcards)

    return len(cards)


def migrate_humanities(data_dir: pathlib.Path | None = None) -> int:
    """Migrate humanities JSON files to Card-array format."""
    d = data_dir or _DATA_DIR
    cards = load_humanities(d)

    main_file = d / "humanities.json"
    raw = json.loads(main_file.read_text("utf-8")) if main_file.exists() else {}
    main_places = set(raw.get("places", {}).keys())

    regional_files = {
        "humanities_films.json": set(),
        "humanities_historical.json": set(),
    }
    for fname in regional_files:
        p = d / fname
        if p.exists():
            regional = json.loads(p.read_text("utf-8"))
            entries = regional.get("places", regional) if isinstance(regional, dict) else regional
            regional_files[fname] = {k for k in entries.keys() if not k.startswith("_")}
...
def migrate_encounters(data_dir: pathlib.Path | None = None) -> int:
    """Migrate encounters.txt to JSON Card-array format."""
    d = data_dir or _DATA_DIR
    cards = load_encounters(d)
    fp = d / "encounters.json"
    _write_cards_json(fp, [_card_to_dict(c) for c in cards])
    return len(cards)


def migrate_people(data_dir: pathlib.Path | None = None) -> int:
    """Migrate people_seed.json to Card-array format."""
    d = data_dir or _DATA_DIR
    cards = load_people(d)
    fp = d / "people_seed.json"
    _write_cards_json(fp, [_card_to_dict(c) for c in cards])
...
def migrate_errands(data_dir: pathlib.Path | None = None) -> int:
    """Migrate errands_letters.json to Card-array format."""
    d = data_dir or _DATA_DIR
    cards = load_errands(d)
    fp = d / "errands_letters.json"
    _write_cards_json(fp, [_card_to_dict(c) for c in cards])
    return len(cards)
