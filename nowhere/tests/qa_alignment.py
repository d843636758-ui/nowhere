...
def generate_report():
    ...
    lines.append(f"| 9. 电台流腐烂 | {len(radio_dead)} | 实锤 |")
    lines.append(f"| 10. 海上死寂 | {len(ocean_zones)} | {'实锤' if any(f['type']=='海上内容黑洞' for f in ocean_zones) else '可疑'} |")
    lines.append(f"| 11. 记忆膨胀 | {len(mem_bloat)} | {'实锤' if any(f['severity']=='实锤' for f in mem_bloat) else '可疑'} |")
    lines.append("")
    ...
    lines.append("5. **电台流腐烂** (实锤): 死亡流需替换或移除, 否则 listen 工具会返回无效地址。")
    lines.append("6. **记忆膨胀** (实锤): geocode._geocode_cache 无 TTL/eviction, 长时间运行会持续增长。")
    lines.append("7. **海上死寂** (可疑): 海洋位置仍保留沿海城市的 localcolor/humanities 内容, 需检查是否符合预期。")
    lines.append("8. **城市vs景区** (可疑): 景区卡需确认城市落点是否合理, 或调整落点坐标。")
    lines.append("9. **物种超分布** (可疑): 需人工二审, 部分可能是数据源本身的分布记录。")
    lines.append("10. **时代错** (人审): 部分可能是故意怀旧, 需人工判断哪些保留哪些删。")
    lines.append("11. **AI编事实** (人审): 需更完整的 ZIM 交叉验证, 当前样本量有限。")
    lines.append("")
